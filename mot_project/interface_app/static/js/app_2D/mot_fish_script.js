class Space_App{
    constructor(n_targets, n_distractors, target_color, distractor_color, boxWidth, boxHeight, radius_min,
                radius_max, speed_min, speed_max, hover_color, img_left, img_right, img_width, img_height)
    {
        ...
        for(let step = 0; step < this.n_targets; step++){
            this.targets.push(new Fish(target_color, img_left, img_right, img_width, img_height, this.speed_min, this.speed_max, step, boxWidth,
                boxHeight,'target', this.targets, hover_color))
        }
        for(let step = 0; step < this.n_distractors; step++){
            this.distractors.push(new Fish(distractor_color, img_left, img_right, img_width, img_height, this.speed_min, this.speed_max,
                step+this.n_targets, boxWidth, boxHeight, 'distractor', this.targets.concat(this.distractors),
                hover_color));
        }
