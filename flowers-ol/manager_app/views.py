...
def home_super(request):
    ...
    time_stamp = (
        p.last_session_timestamp.strftime("%d %b %Y (%H:%M:%S)")
        if p.last_session_timestamp
        else None
    )
    valid_period = p.current_session.get_valid_period(
        p.ref_timestamp, string_format="%d %b %Y (%H:%M:%S)"
    )
...
def reset_user_participant(request):
    ...
    request.user.save()
    django_messages.add_message(
        request, django_messages.SUCCESS, "Participant reset succesfully."
    )
    return redirect(reverse("fork_super"))
