...
class Task(models.Model):
    ...
    extra_json = jsonfield.JSONField(default={}, blank=True)
    actions = jsonfield.JSONField(
        default={}, blank=True
    )  # Must be a csv of valid views
...
class ExperimentSession(models.Model):
    ...
    study = models.ForeignKey(
        Study, to_field="name", null=True, on_delete=models.SET_NULL
    )
    tasks_csv = models.CharField(max_length=200, default="")
    extra_json = jsonfield.JSONField(default=dict, blank=True)
    index = models.IntegerField(
        default=0
    )  # TODO index determines how sessions are ordered (sessions with equal indexes are randomized)
    required = models.BooleanField(default=True)
    wait = jsonfield.JSONField(
        default=dict, blank=True, validators=[validate_timedelta_args]
    )  # must provide valid daytime kwargs, see https://docs.python.org/3/library/datetime.html#datetime.timedelta
    deadline = jsonfield.JSONField(
        default=dict, blank=True, validators=[validate_timedelta_args]
    )  # must provide valid daytime kwargs, see https://docs.python.org/3/library/datetime.html#datetime.timedelta
        ...
        if self.wait and self.deadline:
            if delta(**self.wait) > delta(**self.deadline):
                raise ValidationError(
                    _(
                        f"Cannot set wait={self.wait} and deadline={self.deadline}; wait timedelta cannot be greater than deadline timedelta."
                    )
                )
    ...
    def get_task_list(self):
        ...
        return [
            Task.objects.get(name=task_name) for task_name in self.tasks_csv.split(",")
        ]
    ...
    def get_valid_period(self, ref_timestamp, string_format=""):
        ...
        for i, constraint in enumerate([self.wait, self.deadline]):
            if "days" in constraint.keys():
                valid_period[i] = (ref_timestamp + delta(**constraint)).replace(
                    microsecond=0, second=59 * i, minute=59 * i, hour=23 * i
                )
    ...
    def in_future(self, ref_timestamp):
        ...
        if "days" in self.wait.keys():
            start = (ref_timestamp + delta(**self.wait)).replace(
    ...
    def in_past(self, ref_timestamp):
        ...
        if "days" in self.deadline.keys():
            deadline = (ref_timestamp + delta(**self.deadline)).replace(
...
class ParticipantProfile(models.Model):
    ...
    origin_timestamp = models.DateTimeField(
        ...
        verbose_name="Timestamp for when participant was first created",
    )
    sessions = models.ManyToManyField(ExperimentSession, related_name="study_sessions")
    session_stack_csv = models.TextField(
        default="",
        null=True,
        blank=True,
        verbose_name="csv string of Session PKs",
        validators=[validate_session_stack],
    )
    current_session = models.ForeignKey(
        ExperimentSession, null=True, blank=True, on_delete=models.DO_NOTHING
    )
    last_session_timestamp = models.DateTimeField(
        null=True, blank=True, verbose_name="Date-time of last finished session"
    )
    ...
    def get_next_session_info(self):
        ...
        valid_period = session.get_valid_period(
            self.ref_timestamp, string_format="%d %b %Y (%H:%M:%S)"
        )
        start_info = (
            _("commence le {}").format(valid_period[0])
            if valid_period[0]
            else _("commence à tout moment")
        )
        deadline_info = (
            _("et se termine le {}").format(valid_period[1])
            if valid_period[1]
            else _("et reste ouverte jusqu'à ce que vous la terminiez")
        )
    ...
    def ref_timestamp(self):
        return (
            self.last_session_timestamp
            if self.last_session_timestamp
            else self.origin_timestamp
        )
    ...
    def queue_reminder(self):
        if self.remind and self.sessions.all() and self.session_stack_peek():
            ...
            next_session_start_datetime = next_session.get_valid_period(
                ref_timestamp=self.ref_timestamp
            )[0]
            message_template = render_to_string(
                self.study.reminder_template,
                {
                    "CONTEXT": {
                        "username": self.user.username,
                        "valid_period": next_session_start_datetime,
                        "tasks": next_session.get_task_list,
                        "study_link": "http://flowers-mot.bordeaux.inria.fr/study={}".format(
                            self.study.name
                        ),
            ...
            index = list(
                ExperimentSession.objects.filter(study=self.study).values_list(
                    "pk", flat=True
                )
            ).index(next_session.pk)
            send_delayed_email(
                to=self.email,
                ...
                schedule=datetime.datetime(
                    year=next_session_start_datetime.year,
