    ...
    for i in no_spaces.split(","):
        if not i.isdigit():
            raise ValidationError(
                (
                    f"Session stack is not a valide csv string. Check how session stack is creater or assigned"
                )
            )
...
def validate_timedelta_args(value):
    if value:
        ...
        if any(problems):
            raise ValidationError(
                'this is a JSON field that must have at most 1 key-value pair; key must be either "days" or "minutes" (case-sensitive); value must be positive nonzero number'
            )
