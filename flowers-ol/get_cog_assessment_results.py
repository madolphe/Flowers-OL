...
p = argparse.ArgumentParser(
    "Connector to django DB for cognitive assessment",
...
p.add_argument(
    ...
    "--export_all",
    action="store_true",
    help="Boolean flag to export all the cog assessments to CSV",
)
args = p.parse_args()
...
def format_dictionnary(dataset, has_condition=True):
    for participant, results in dataset.items():
        ...
        for result in results:
            new_result = {}
            if has_condition:
                new_result[result.cognitive_task.name] = [
                    result.idx,
                    result.results,
                    result.status,
...
def export_to_csv_for_task(
    dataset, task_name, has_condition=True, study="default_study"
):
    """
            ...
            if not problem:
                ...
                for column, value in dict_results.items():
                    try:
                        ...
                        print(
                            f"First participants results keys for this activty are: {dict_to_export.keys()}"
                        )
                ...
                print(
                    f"First participants results keys for this activy are: {dict_to_export.keys()}"
                )
                print(f"This participant presents keys: {dict_results.keys()}")
...
def get_age_gender(df):
    ...
    for key in df.keys():
        ...
        if participant.study.name == study:
            age.append(
                ...
                - int(
                    Answer.objects.get(
                        participant=participant, question__handle="prof-mot-12"
                    ).value
                )
            )
            gender.append(
                int(
                    Answer.objects.get(
                        participant=participant, question__handle="prof-mot-2"
                    ).value
                )
            )
    # 1 is male - 0 female
    ...
    print(f"Mean age: {sum(age) / len(age)}, nb male: {sum(gender)}")


def get_dataset(study, has_condition=True):
    """Run of all functions needed for exporting the data"""
    all_cognitive_results = CognitiveResult.objects.filter(
        participant__study__name=study
    )
    # Create a dictionnary with participant_id as key and a list of CognitiveResults objects as values
    ...
    completed_session, half_completed_session, other = (
        count_number_of_completed_session(dataset)
    )
    # Reformat this dictionnary to get {"participant_id":[{'task_name':[idx, {results}, status], ...., }]}
    completed_session = format_dictionnary(
        completed_session, has_condition=has_condition
    )
...
def treat_prolific_data(df):
    ...
    for key in completed_session.keys():
        ...
        row = demog_df[
            demog_df["participant_id"]
            == str(ParticipantProfile.objects.get(user__id=key).user)
        ]
        row["id"] = key
...
def get_csv_for_each_task(completed_session, study="default"):
    ...
    for task_name in task_list:
        ...
        export_to_csv_for_task(
            dataset, task_name, has_condition=has_condition, study=study
        )
...
def get_vgq(participant_keys):
    ...
    for participant_key in participant_keys:
        ...
        answers = Answer.objects.filter(
            participant=participant, question__instrument="VGQ"
        )
