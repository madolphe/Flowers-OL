class progressBar{
    constructor(idx_active_item){
        ...
        this.x_active_item = this.box_x + (this.idx_active_item+1)*this.step_size;
    ...
    draw_text(){
        ...
        text(this.percentage_text, this.box_x + this.box_width + this.margin_text, this.middle_box_y);
        pop();
    }
    draw_circles(){
        for(let i=1; i<=this.nb_ticks; i++){
            push();
            if((i-1)<this.idx_active_item){
                fill('green');
                imageMode(CENTER);
    ...
    draw_backbone_bar(){
        ...
        strokeWeight(this.stroke_weight);
        fill("#25ae88");
        rect(this.box_x + this.step_size, this.middle_box_y-this.progress_bar_width/2,
        ...
        rect(this.box_x  + this.step_size * (this.idx_active_item ), this.middle_box_y-this.progress_bar_width/2,
             this.box_x + this.box_width - this.step_size, this.middle_box_y+this.progress_bar_width/2)
        pop();
    }
    draw_active_task(){
        push();
