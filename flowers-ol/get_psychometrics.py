...
def create_df_from_participant(participant_answers):
    ...
    for answer in participant_answers:
        all_answers.append(
            [
                answer.participant.user_id,
                answer.participant.extra_json["condition"],
                answer.question.component,
                answer.question.instrument,
                answer.question.handle,
                answer.session.index,
                answer.value,
            ]
        )
    return all_answers
...
def format_questionnaire(df_tlx):
    ...
    for participant_id in participants_id:
        ...
        for session_id in sessions_id:
            row_participant_session_id = {
                "id_participant": participant_id,
                "session_id": session_id,
            }
            participant_answers_session = participant_answers[
                participant_answers["session_id"] == session_id
            ]
            for id, ans in participant_answers_session.iterrows():
                ...
                if ans.component in row_participant_session_id:
                    row_participant_session_id[ans.component] = (
...
def display_cols_value(
    df_baseline_mean, df_baseline_std, df_zpdes_mean, df_zpdes_std, dir
):
    for col in df_baseline_mean.columns:
        plt.errorbar(
            df_baseline_mean[col].index,
            df_baseline_mean[col],
            yerr=df_baseline_std[col],
            label="baseline",
            color="blue",
        )
        ...
        plt.errorbar(
            df_zpdes_mean[col].index,
            df_zpdes_mean[col],
            yerr=df_zpdes_std[col],
            label="zpdes",
            color="red",
        )
        plt.legend()
...
def plot_boxplots(df, subscales, instrument):
    ...
    for col in subscales:
        sub_dict = (
            df_tmp[df_tmp["component"] == col]
            .groupby(["id", "session_id", "condition"])
            .mean()
            .reset_index()
        )
...
def plot_questionnaire(instrument, subscales):
    ...
    # get format:
    df_baseline_mean = (
        format_questionnaire(df_tlx_baseline).groupby("session_id").mean()
    )
    ...
    display_cols_value(
        df_baseline_mean, df_baseline_std, df_zpdes_mean, df_zpdes_std, instrument
    )
